...
public class BBoard extends JFrame
{
...
//			System.out.println(icon);
		...
		{
			System.out.println("Sono su controllo Click");
...
/*		
		public void showButtons(LinkedList list)
		{
			...
			{	
				...
				icon=new ImageIcon();
//				Mbuttons[x][y].removeMouseListener(listener);
				if((Block)blist.get(i) instanceof Flower)
				{
					icon=((Flower)blist.get(i)).getIcon();
					((Flower)board.getElement(x, y)).setValue(-3);
//						for(int l=0;l<bRows;l++)
//							for(int k=0;k<bColumns;k++)
//							 Mbuttons[l][k].removeMouseListener(listener);
				...
				else if((Block)blist.get(i) instanceof Number)
					{
						...
						((Number)board.getElement(x, y)).setValue(-4);
//						Mbuttons[x][y].removeMouseListener(listener);
					}
